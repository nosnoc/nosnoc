...
% digits(64)
scenario_name = 'irk_with_fesd_smoothed';
moving_finite_elements_on = 1;
...
d_vec = [1 2 3 4 5];
% d_vec = 4;
M_vec  = round(logspace(1.5,3.4,10))+7;
...
% h_sim_vec = logspace(log10(0.001),log10(0.2),10);
nn = logspace(log10(11),log10(2001),10);
nn = round(nn);
nn(mod(nn,2)==0) = nn(mod(nn,2)==0)+1
% h_sim_vec  = T./[11 31 61 101 301];
h_sim_vec = T./nn;
%% preproces of step-size to avoid exact switch detection
% h_sim_vec = logspace(log10(0.0005),log10(0.2),10);
N_sim_vec = (T./h_sim_vec);
...
settings.mpcc_mode = 2;                    % 1 - extact, 2 - smooth  ,3 -relax , 4 - penalty, 5 - l1 penalty
...
settings.gamma_h = 2;                    % how much can h addapt
...
% M_vec_log = log(M_vec);
for ii = 1:length(d_vec)
    mean(abs(diff(errors_all_experiments_log(ii,:))/0.5))
end
