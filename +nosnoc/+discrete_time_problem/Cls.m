classdef Cls < vdx.problems.Mpcc
    ...
    methods
        ...
        function create_variables(obj)
            ...
            obj.p.rho_path = {{'rho_path_p',1}, opts.rho_path};
            obj.p.T = {{'T',1}, opts.T};
                ...
                if model.friction_exists
                    switch opts.friction_model
                        ...
                        case 'Conic'
                            ...
                            obj.w.gamma(ii,1:opts.N_finite_elements(ii),1:(opts.n_s)) = {{'gamma', dims.n_c}, 0, opts.ub_gamma, opts.initial_gamma};
                            obj.w.beta(ii,1:opts.N_finite_elements(ii),1:(opts.n_s)) = {{'beta', dims.n_c}, 0, opts.ub_beta, opts.initial_beta};
                            switch opts.conic_model_switch_handling
                                case 'Plain'
                                    % no extra vars
                                case 'Abs'
        ...
        function generate_direct_transcription_constraints(obj)
            ...
            for ii=1:opts.N_stages
                    ...
                    switch opts.rk_representation
                        case RKRepresentation.integral
                            ...
                            for kk=1:opts.n_s
                                ...
                                fj = s_sot*dcs.f_x_fun(x_ijk, z_ijk, z_alg_f_x_ijk/h_rescale, u_i, v_global, p);
                                qj = s_sot*dcs.f_q_fun(x_ijk, z_ijk, u_i, v_global, p); %TODO@anton: Allow objective depend on forces;
                                ...
                                obj.g.lift_v(ii,jj,kk) = {dcs.g_lift_v_fun(x_ijk, z_ijk, z_alg_f_x_ijk/h_rescale, u_i, v_global, p)};

                                x_ij_end = x_ij_end + opts.D_rk(kk+1)*x_ijk;

                                if opts.g_path_at_stg
                                    obj.g.path(ii,jj,kk) = {dcs.g_path_fun(x_ijk, z_ijk, u_i, v_global, p), model.lbg_path, model.ubg_path, relax_path_struct};
                                end
                            ...
                            if ~opts.g_path_at_stg && opts.g_path_at_fe
                                obj.g.path(ii,jj) = {dcs.g_path_fun(x_ijk, z_ijk, u_i, v_global, p), model.lbg_path, model.ubg_path, relax_path_struct};
                            end
                ...
                if ~opts.g_path_at_stg && ~opts.g_path_at_fe
                    ...
                    obj.g.path(ii) = {dcs.g_path_fun(x_i, z_i, u_i, v_global, p), model.lbg_path, model.ubg_path, relax_path_struct};
                end
                ...
                % Clock Constraints
                if opts.equidistant_control_grid && opts.use_fesd
            ...
            p_stage_end = obj.p.p_time_var(obj.opts.N_stages);
            % Terminal cost
            obj.f = obj.f + dcs.f_q_T_fun(x_end, z_end, v_global, [p_global;p_stage_end]);
                            ...
                            switch opts.friction_model
                                case 'Polyhedral'
                                    for kk=1:(opts.n_s)
                                        ...
                                        G_ij = vertcat(G_ij, {lambda_tangent_ijk});
                                        H_ij = vertcat(H_ij, {delta_d_ijk});
                                        G_ij = vertcat(G_ij, {beta_d_ijk});
                                        H_ij = vertcat(H_ij, {gamma_d_ijk});
                                    end
                                case 'Conic'
                                    for kk=1:(opts.n_s)
                                        beta_ijk = obj.w.beta(ii,jj,kk);
                                        gamma_ijk = obj.w.gamma(ii,jj,kk);
                                        G_ij = vertcat(G_ij, {beta_ijk});
                                        H_ij = vertcat(H_ij, {gamma_ijk});
                                    end
                                    switch opts.conic_model_switch_handling
                                        case 'Plain'
                                            % no extra expr
