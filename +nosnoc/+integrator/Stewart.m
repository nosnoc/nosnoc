classdef Stewart < handle
    ...
    methods
        function obj = Stewart(model, opts, integrator_opts)
            import casadi.*
            obj.model = model;
            obj.opts = opts;
            ...
            switch class(model)
              case "nosnoc.model.Pss"
                ...
                % 1) z_l for a given active set and corresponding ODE
                % 2) functions for LPC 
        ...
        function [t_grid,x_res,t_grid_full,x_res_full] = simulate(obj, plugin, extra_args)
            ...
            for ii=1:opts.N_sim
                ...
                %                                                % TODO(@armin): check
                %                                                % are events
                %                                                % supported for sundails integrators
                %     if ~exist('F', 'var')
                %         F = ode;
                ...
                % TODO(@armin)  - ADD simulation of sensitivity equations
                % in parallel and their extraction! (+ sensitivity jump  formula)
                % TODO(@armin)  - ADD option to pass a fixed integration grid
                % TODO(@armin)  - return index set of t and switching times  (subset of tgrid_full)
                t_current = t_sim(end);
